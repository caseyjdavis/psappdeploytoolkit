...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
        ...
        Execute-MSI -Action Install -Path 'sw_lic_full_installer.msi' ## Uncomment if you need parameters -AddParameters ## if you have more to pass to the MSI besides /qb and norestart
        ...
        Write-Log "Copying Shockwave DLL to Firefox Plugins Folder"
        If (Copy-Item "$env:windir\system32\Adobe\Director\np32dsw.dll" -Destination "$env:ProgramFiles(x86)\Mozilla Firefox\browser\components" -Force) {Write-Log "Adobe Shockwave np32dsw.dll is copied"}
        ...
        Execute-MSI -Action Uninstall -Path 'sw_lic_full_installer.msi'
