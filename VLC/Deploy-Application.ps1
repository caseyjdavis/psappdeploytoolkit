...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
        ...
        Execute-Process -Path 'vlc-2.2.4-win32.exe' -Parameters '/S /NCRC'
        ...
        #Cleans up icons
        If (Test-Path "$envCommonDesktop\VLC media player.lnk") {Remove-Item "$envCommonDesktop\VLC media player.lnk" -Force}
        ...
        Start-Process "${env:ProgramFiles(x86)}\VideoLAN\VLC\uninstall.exe" -ArgumentList "/S _?=${env:ProgramFiles(x86)}\VideoLAN\VLC"
