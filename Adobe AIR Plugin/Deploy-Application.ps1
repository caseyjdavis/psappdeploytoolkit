...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
        ...
        Execute-Process -Path 'AdobeAIRInstaller.exe' -Parameters '-silent -eulaAccepted'
        ...
        # Set registry key to disable automatic updating of Adobe AIR
        If (!(Test-RegistryValue -Key 'HKLM:\SOFTWARE\Policies\Adobe\AIR\' -Value 'UpdateDisabled' -Verbose)){
              Set-RegistryKey -Key 'HKLM:\SOFTWARE\Policies\Adobe\AIR\' -Name 'UpdateDisabled' -Value 1 -Type Dword; Write-Log "Set Adobe AIR Registry Key to Disable Autoupdate" }
        ...
        Execute-Process -Path 'AdobeAIRInstaller.exe' -Parameters '-uninstall'
